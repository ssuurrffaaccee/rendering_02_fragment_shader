...
public:
  ...
  virtual std::vector<VertexStageData>
  ...
  virtual std::tuple<Vec<int, 2>, Color, float>
  process_fragment(const FragmentStageData &frag) = 0;
...
public:
  ...
  void register_(const std::string &name,
    ...
    CHECK(shader_builders_.find(name) ==
          shader_builders_.end())
